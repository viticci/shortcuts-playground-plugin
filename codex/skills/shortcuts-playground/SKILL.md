...
Behavior:
- Generates brand-new random shortcuts under `--output-dir` when provided, otherwise `~/Agent/Shortcuts Playground/<YYYY-MM-DD>/random-mixed-actions-<runid>/`.
...
52. **Set Name best practice**: Use `is.workflow.actions.file.rename` with both `WFFile` and `WFNewFilename` (for example, `Test.txt`) so downstream actions receive a renamed file object. Do not wire the same magic variable or action output into both Set Name's `WFFile` and a later `Delete File` / `Move File`; after Set Name, the source file reference cannot be reliably resolved to the original on-disk file. For rename-and-remove workflows, capture the original file path as text before Set Name with Get Details of Files -> File Path and feed that captured path to Delete/Move

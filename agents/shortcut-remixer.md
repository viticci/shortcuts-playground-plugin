...
- **Add**: which new actions, at which positions, with which UUIDs.
- **Modify**: which existing actions (by index + UUID), which specific parameters to change, what the new values are.
...
Use the `Write` tool to write a COPY of the source XML to `<OUTPUT_DIR>/drafts/<new name>.xml`. The content of this initial `Write` must be byte-identical to the source file — your edits happen in step 9, not here. The `PostToolUse` hook will run the validator on this Write; treat any errors on the initial copy as informational (they're inherited from the source).

Why a straight copy first? It gives the `Edit` tool a stable starting point with exact text to match, and it lets the validator tell you which pre-existing issues the source already has.

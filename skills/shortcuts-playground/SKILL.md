...
4. [VARIABLES.md](VARIABLES.md), [CONTROL_FLOW.md](CONTROL_FLOW.md), and [FILTERS.md](FILTERS.md) for wiring patterns
5. [ICONS_AND_COLORS.md](ICONS_AND_COLORS.md), [PARAMETER_TYPES.md](PARAMETER_TYPES.md), and [EXAMPLES.md](EXAMPLES.md) for implementation details
...
- 40 Weather Detail cases (20 valid + 20 invalid)
...
Behavior:
- Generates brand-new random shortcuts under `$CLAUDE_PLUGIN_OPTION_OUTPUT_DIR/<YYYY-MM-DD>/random-mixed-actions-<runid>/` (or `~/Documents/Shortcuts Playground/` if `output_dir` was not set when the plugin was enabled).
...
17. **Conditional inputs**: For `is.workflow.actions.conditional` with `WFControlFlowMode = 0`: numeric conditions (codes 0-3) use implicit input — do NOT set `WFInput`, place the source action immediately before the If; string/existence conditions (codes 4, 5, 8, 9, 99, 100, 101, 999) require explicit `WFInput` with `Type=Variable` wrapper. Modes 1/2 omit inputs entirely. For JSON booleans, compare numerically (`1`/`0`) not string `"true"`/`"false"`; treat JSON `null` as empty/nothing. See BEST_PRACTICES.md condition code table for the complete reference.
...
38. **Weather detail wiring**: For `is.workflow.actions.properties.weather.conditions`, set both `WFInput` and `WFContentItemPropertyName`, keep `WFContentItemPropertyName` concrete (never placeholder `Detail`), and wire `WFInput` directly to an ActionOutput from `is.workflow.actions.weather.currentconditions` / `is.workflow.actions.weather.forecast` (no named-variable hop)
